...
module inMediumWidth
  ...
  !****************************************************************************
  !****g* inMediumWidth/num_MonteCarlo_Points
  ! SOURCE
  !
  integer, save :: num_MonteCarlo_Points = 2000
  !
  ! PURPOSE
  ! number of monte-carlo points for the sampling of the phase space
  !
  ! usual values: 2000 for baryons, 250 for mesons
  !****************************************************************************
...
contains
  ...
  subroutine readInput
    ...
    NAMELIST /inMediumWidth/ debugFlag,maxRes,minRes,writeLocal, &
         maxMes,minMes,max_absP_Mes,num_MonteCarlo_Points,cmin,cmax
  ...
  !
  ! OUTPUT
  ! * Files "inMediumWidth/InMediumWidth.particleID_i_s.dat.bz2"
  !   with i=1,2 and s=1,2(=inMediumParameterset)
  !****************************************************************************
  subroutine tabulate_inMediumWidth_baryons()
    ...
    ! here (in contrast to baryonWidthMedium_tables):
    ! * we only store it for the actual ID
    ! * it is double precision, not real*4 (output file is formatted, so
    !   the internal storage does not matter)
    ...
    if (readInput_flag) then
       ...
       ! The following construction with a dummy variable is necessary
       ! to avoid a recursive I/O operation:
    ...
    do ID=max(minRes,1),min(maxRes,nres+1)
       ...
       do iAbsP=0,numSteps_absP
          ...
          do iMass=0,numSteps_mass
             ...
             if (mass.lt.hadron(ID)%minmass) cycle
             ...
             do iRhoN=0,numSteps_rhoN
                ...
                do iRhoP=0,numSteps_rhoP
                   ...
                   widthTable(iAbsP,iMass,iRhoN,iRhoP,showPauli) &
                        = calcPauliBlockedDecayWidth(ID,absP,mass,rhoN,rhoP)
                   widthTable(iAbsP,iMass,iRhoN,iRhoP,showColl) &
                        = calcCollBroadening_Bar(ID,absP,mass,rhoN,rhoP)
                end do
             end do
             ! for doku:
             write(112,*) ID,absP,mass,&
                  widthTable(iAbsP,iMass,0,0,1),&
                  widthTable(iAbsP,iMass,0,0,2),&
                  widthTable(iAbsP,iMass,numSteps_rhoN,numSteps_rhoP,1),&
                  widthTable(iAbsP,iMass,numSteps_rhoN,numSteps_rhoP,2)
          end do
       end do
       ...
       do i=1,2
          ...
          write(buf,'(A,7I6)')'#', numSteps_absP,numSteps_mass, &
               numSteps_rhoN,numSteps_rhoP,num_MonteCarlo_Points, &
               get_min_charge_loop(),get_max_charge_loop()
          call bzWriteLine(f,buf)
          write(buf,'(A,5E15.4)')'#', max_absP, max_rhoP, max_rhoN, &
               min_mass(ID),max_mass(ID)
          call bzWriteLine(f,buf)
          do iAbsP=0,numSteps_absP
             do iMass=0,numSteps_mass
                do iRhoN=0,numSteps_rhoN
  ...
  subroutine tabulate_inMediumWidth_mesons()
    ...
    use mesonWidthMedium_tables, only: numSteps_rhoN,numSteps_rhoP,&
         numSteps_absP,numsteps_mass,max_rhoP,max_rhoN
    use bzip
    ...
    do ID=max(pion,minMes),min(pion+nmes-1,maxMes)
             ...
             do iRhoN=0,numSteps_rhoN
                ...
                do iRhoP=0,numSteps_rhoP
                   ...
                   widthTable(iAbsP,iMass,iRhoN,iRhoP)=&
                            & calcCollBroadening_Mes(ID,absP,mass,rhoN,rhoP)
                end do
             end do
       ...
       f = bzOpenW(trim(fileName))
       write(buf,'(A,7I6)')'#', numSteps_absP,numSteps_mass,&
            numSteps_rhoN,numSteps_rhoP,num_MonteCarlo_Points,cmin,cmax
       call bzWriteLine(f,buf)
       write(buf,'(A,1P,5E15.4)')'#', max_absP_Mes,max_rhoP,max_rhoN,&
            min_mass,max_mass
       call bzWriteLine(f,buf)
       do iAbsP=0,numSteps_absP
          do iMass=0,numSteps_mass
  ...
  !****************************************************************************
  !****f* inMediumWidth/calcPauliBlockedDecayWidth
  ! NAME
  ! function calcPauliBlockedDecayWidth(particleID,momentumLRF,mass,rhoN,rhoP)
  ! result(gammaDecay)
  !
  ! PURPOSE
  ...
  ! * real, intent (in)   :: rhoN,rhoP   -- proton, neutron density in fm^-3
  !
  ! OUTPUT
  ! * real :: gammaDecay
  !****************************************************************************
  function calcPauliBlockedDecayWidth(particleID,momLRF,mass,rhoN,rhoP) &
       result(gammaDecay)

    use random
    use master_1Body, only: decayParticle
    use particleProperties, only: validcharge,hadron
    ...
    real :: gammaDecay ! function value

    type(particle)                   :: part
    type(particle),dimension(1:10)   :: finalState

    logical :: pauliBlocking, collisionFlag, finalFlag
    real :: gamma, fermi_n, fermi_p
    integer :: i,j

    gammaDecay=0.
    if(hadron(particleID)%stability == 0) return
  ...
  end function calcPauliBlockedDecayWidth




  !****************************************************************************
  !****f* inMediumWidth/calcCollBroadening_Bar
  ! NAME
  ! function calcCollBroadening_Bar(particleID,momentumLRF,mass,rhoN,rhoP) RESULT(gcoll)
  !
  ! PURPOSE
  ! * Returns the in-medium width of baryons according to
  ...
  !****************************************************************************
  function calcCollBroadening_Bar(particleID,momLRF,mass,rhoN,rhoP,monte_in,gcoll_elastic_out) RESULT(gcoll)
    use particleDefinition
    use random
...
!       num_MonteCarlo_Points_local= 250 !for speed up
       num_MonteCarlo_Points_local= num_MonteCarlo_Points
    end if
  ...
  !****************************************************************************
  !****f* inMediumWidth/calcCollBroadening_Mes
  ! NAME
  ! function calcCollBroadening_Mes(particleID,momentumLRF,mass,rhoN,rhoP) RESULT(gcoll)
  !
  ! PURPOSE
  ...
  ! * real :: gcoll
  !****************************************************************************
  function calcCollBroadening_Mes(particleID,momLRF,mass,rhoN,rhoP) RESULT(gcoll)
    use particleDefinition
    use random
    use constants, only: GeVSquared_times_mb, mN
    use particleProperties, only: validCharge
  ...
  end function calcCollBroadening_Mes
  ...
  !****************************************************************************
  pure real function fermiMom(rho)
