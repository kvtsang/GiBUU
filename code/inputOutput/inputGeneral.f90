...
module inputGeneral
  ...
  !****************************************************************************
  !****g* inputGeneral/onlyFirstEvent
  ! PURPOSE
  ! Switch to turn off all final state colisions by only allowing particles
  ! to suffer a 2-body od 3-body collisions, if their field '%firstEvent=0'
  !
  ! SOURCE
  !
  logical, save :: onlyFirstEvent = .false.
  !****************************************************************************
  ...
  ! pion potential, 2N pion absorption and Delta inmed broadening
  !
  ! SOURCE
  !
  logical, save :: master_piDelta_inmed = .false.
  ! NOTES
  ! This is a master switch for pi-Delta inmed changes.
  ! If set to .true. it turns on all the inmedium pion-Delta corrections, i.e.:
  ! * baryonWidthMedium/mediumSwitch=.true.,
  ! * baryonWidthMedium/mediumSwitch_Delta=.true.,
  ! * s-wave pion absorption master_3Body/s_wave_abs = .true, and
  ! * pion Potential mesonPotential/pionpotSwitch = 4
  ! If .false., the listed switches can be set independently
  !****************************************************************************

  !****************************************************************************
  !****g* inputGeneral/version
  ! PURPOSE
  ! Indicator, for which code version this jobcard is suitable.
  !
  ! possible value (at the moment):
  ! * 2025: for the use with the release version v2025
  !
  ! if not given, or wrong value, the code will stop execution
  !
  ! SOURCE
  !
  integer, save, private :: version = -1
  !****************************************************************************
...
contains
  ...
  subroutine readInputGeneral
    ...
    use masterPiDelta, only: setMasterPiDelta
    ...
    !**************************************************************************
    !****n* inputGeneral/inMedium
    ! NAME
    ! NAMELIST /InMedium/
    ! PURPOSE
    ! contains the master switch for pion-Delta in medium properties:
    ! * master_piDelta_inmed
    !**************************************************************************
    NAMELIST /InMedium/ master_piDelta_inmed
    ...
    ! workaraound for ifort 2023.1 (=version 2021.9.0 Build 20230302_000000)
    ! without the inquire, the following rewind gives ios=499, even with
    ! file piped in
    inquire(5,IOSTAT=ios)
    ...
    case default
       write(*,*)
       write(*,*) "This jobcard is not prepared to run with the actual code version."
       write(*,*) "Please give explicitely 'version=2025' in this namelist."
       write(*,*)
       call Traceback('This info is crucial. STOP!')
    end select
    ...
    write(*,'(A,i3,A,A)') ' Eventtype   =  ',eventtype,&
         '    = ',trim(cEventType(eventtype))

    if (useProductionPos) then
       select case (eventType)
       case (HiLepton,RealPhoton)
          ! do nothing
       case default
    ...
    if (NumEnsembles>0 .and. NumEnsembles<100) then
       write(*,*) 'It is a bad idea to use #ensembles < 100 !'
       write(*,*) 'To enforce it anyway, give the number with a minus sign.'
       call Traceback()
    end if
    NumEnsembles = abs(NumEnsembles)
    ...
    write(*,*) 'Number runs/energy    =',num_runs_sameEnergy
    ...
    do i=-10,5
       if (DoPrLevel(i).neqv.DoPrLevelDefault(i)) then
          write(*,'(" DoPr(",i3,") changed: ",L1," --> ",L1)') &
               i,DoPrLevelDefault(i),DoPrLevel(i)
       end if
    end do
    write(*,*)
    ...
    call Write_ReadingInput("InMedium",0)
    rewind(5)
    read(5,nml=InMedium,IOSTAT=ios)
    call Write_ReadingInput("InMedium",0,ios)

    call setMasterPiDelta(master_piDelta_inmed)
    write(*,*) 'master_piDelta_inmed=',master_piDelta_inmed

    call Write_ReadingInput("InMedium",1)
