...
!******************************************************************************
  ...
  subroutine Root_write_particle(this, part)
    ...
    call rootaddparticle(KF, part%ID, part%number, part%history, &
      part%charge, part%mass, &
      part%mom(1), part%mom(2), part%mom(3), part%mom(0), &
      pos(1), pos(2), pos(3), &
      part%event(1), part%event(2), part%firstEvent, &
      part%lastCollTime, part%prodTime, part%formTime, & 
      part%scaleCS, part%offshellPar)
...
!******************************************************************************
  ...
  ! ensemble.
  ! (This is strictly speaking only true in parallel ensemble mode or in
  ! perturbative runs, where the real particles are frozen. In other cases use
  ! this output method with care).
  !****************************************************************************
  subroutine write_real(this, parts)
    use particleDefinition
  ...
  ! PURPOSE
  ! Do the allocation stuff for the Particle Info List needed for the
  ! perturbative output
  !****************************************************************************
  subroutine ValueListAllocate(ValueList, n1)
    use particlePointerListDefinition
  ...
  ! * Events with no particles in the output (e.g. due to detector cuts)
  !   are not included included in the list and thus produce no output.
  !****************************************************************************
  subroutine write_pert(this, parts)
    use particleDefinition
    use particlePointerListDefinition
    ...
    integer :: i,iEns,iPart, iFE,iiFE,iiiFE, NUP, iEvent
    ...
    do iEns = 1,size(Parts,dim=1)
       PartLoop:do iPart = 1,size(Parts,dim=2)
          ...
          iiFE = PILIndex_PUT(IndexList, iFE, 'EventOutput_pert')
          if (iiFE>0) then
    ...
    do iiiFE=1,IndexList%nEntry
       iiFE = IndexList%Entry(iiiFE) ! use IndexList as lookup table
