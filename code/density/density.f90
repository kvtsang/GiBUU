...
!!$  end function hadronicPotEnergy
  ...
  subroutine init
    ...
    character(22), dimension(0:4), parameter :: cSwitch = (/ &
         'zero                  ', &
         'dynamic density       ', &
         'static density        ', &
         'resting matter        ', &
         'dynamic density in box' /)
    ...
    case (0:4)
       write(*,'(A,I2,2A)') ' densitySwitch:       ',densitySwitch,': ',&
            cSwitch(densitySwitch)
    case default
       write(*,'(A,I2,A)') ' densitySwitch:       ',densitySwitch,': WRONG!'
    ...
    write(*,'(A,L2)') ' linearInterpolation: ',linearInterpolation
    if (setnewsmearing) then
       write(*,*) 'new SmearingWidth: ', newsmearing
    end if
  ...
  real function FermiMomAt(pos,charge)
    ...
    case (2) !Static density of a resting target
       ...
       if (nuc%ReAdjustForConstBinding) then
          dens%proton  = dens%proton/nuc%fac(1)
          dens%neutron = dens%neutron/nuc%fac(2)
