! This is a program to sum different Hist2DMP (written in binary mode)
program BinSumHist2DMP
  ...
  do
     ...
     if (nDirs==0) then
        call fetchHist2DMP(hSum,file=trim(dir)//trim(arg), &
             add=addFak,mul=mulFak, flagOK=flagOK)
        write(*,*) 'add,mul = ',addFak,mulFak
     else
        call fetchHist2DMP(hTmp,file=trim(dir)//trim(arg), flagOK=flagOK)
     end if
     if (.not.flagOK) cycle

     if (nDirs>0) then
        call sumHist2DMP(hSum, hTmp)
     end if
  ...
  call writeHist2DMP_Gnuplot(hSum, file=trim(arg0), add=addFak,mul=mulFak/nDirs,dump=.true.)

end program BinSumHist2DMP
