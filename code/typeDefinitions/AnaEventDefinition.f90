...
module AnaEventDefinition
  ...
  ! Hadrons listed here can be included in detailed analyses,
  ! see  namelist &detailed_diff
  ...
  !
  ! NOTE:
  ! all the stable particles also have to be listed in subroutine event_add
  ...
  type tAnaEvent
     sequence
     type(tParticleList) :: Parts               ! particles in the event
     integer,dimension(1:14,-2:2) :: nParts =0  ! counters for stable particles

  End type tAnaEvent
  ! NOTES
  ! The field nParts includes the multiplicities of particles.
