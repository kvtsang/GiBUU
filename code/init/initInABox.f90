...
!
! Will be used as first step of initializing the particle vectors for
! InABox_pion and InABox_delta
!******************************************************************************
module initInABox
...
contains
  ...
  subroutine initInput
    ...
    use densityModule, only: getDensitySwitch,setDensityInput,densityAt
    ...
    !**************************************************************************
    NAMELIST /InABox/ proton_Density, neutron_Density, fermiMotion, &
         temp, energy_density, standing_wave_number

    call Write_ReadingInput('InABox',0)
    rewind(5)
    read(5,nml=InABox,iostat=ios)
    call Write_ReadingInput('InABox',0,ios)
    ...
    select case (getDensitySwitch())
    case (1)
       ! ok
    case (3)
       write(*,'(A)') 'WARNING : Set parameters densityInput_XXX in '// &
            'densityModule to the values which are given here!!'
       call setDensityInput(proton_density,neutron_density)
    case default
       write(*,*) 'Wrong densitySwitch for box-calculations:', &
            getDensitySwitch()
       call TRACEBACK('Only 1 or 3 allowed!')
    end select

    density=densityAt((/0.,0.,0./))
    write(*,'(A,4F8.3)') '  proton  density with densityAt(0)=',density%proton
    write(*,'(A,4F8.3)') '  neutron density with densityAt(0)=',density%neutron

    call Write_ReadingInput('InABox',1)
  ...
  subroutine initializeInABox(teilchen)
    ...
    ! volume: factor two for each dimension, because box goes from
    ! -gridsize to +gridsize in each direction
    ...
    do i=1,size(teilchen,dim=1)  ! Loop over all ensembles
       ...
       do k=1,numberNeutrons+numberProtons ! Loop over all particles in the box
          ...
          call setToDefault(teilchen(i,index))
          call setNumber(teilchen(i,index)) ! give each particle a unique number
          ! each particle needs a different %event,
          ! otherwise no collisions can happen
          ...
          call chooseCharge()
          Teilchen(i,index)%pos(1) = rnFlat(-1.,1.) * gridSize(1)
          Teilchen(i,index)%pos(2) = rnFlat(-1.,1.) * gridSize(2)
  ...
  contains
    ...
    subroutine chooseMomentum(z)
      ...
      if (fermiMotion) then
         ...
         if (temp>0.) then
            ! positive temperature: sample from Fermi distribution
         ...
         Teilchen(i,index)%mom(1:3) = p * rnOmega()
      else
         ! particles at rest
         p = 0.
         Teilchen(i,index)%mom(1:3) = 0.
      end if
      Teilchen(i,index)%mom(0) = sqrt(p**2+mN**2)
  ...
  subroutine BoostToEps(Teilchen)
    ...
    do i=1,nEns  ! Loop over all ensembles
       do j=1,size(Teilchen,dim=2)  ! Loop over all particles
          ...
          call lorentz(betaV,Teilchen(i,j)%mom)

          SumE = SumE + Teilchen(i,j)%mom(0)
