...
contains
 ...
 !   Q2 = 4. * E_in * (E_in - nu) * sin(theta/2.)**2
  ...
  subroutine nuNucleon(process_ID,IP,Q2,nuclcharge,E_in,theta,nu,sigma_res,sigma_nr)
    ...
    if (sigma < 0) then
       if(abs(sigma) >  5.e-14) write(*,*) 'sigma < 0 ','Q2=',Q2,'nu=',nu,'E_in=',E_in,'sigma=',sigma
       sigma = 0.
    end if
...
!!$  end subroutine resder
  ...
  SUBROUTINE RESMODD(w2,q2,xval,sigres,sig,F1_res,F1_nr)
    ...
    ! function arguments:
    real*8 :: w2, q2
    real*8, intent(out) :: sig, F1_res, F1_nr
    real, dimension(2,7), intent(out) :: sigres
    real*8, intent(in) :: xval(50)

    real*8, parameter ::   mp = 0.9382727
    real*8, parameter ::   mpi = 0.135
    real*8, parameter ::   mpi2 = mpi*mpi
    real*8, parameter ::   meta = 0.547
    real*8, parameter ::   mp2 = mp*mp
    real*8, parameter ::   pi = 3.141593
    real*8, parameter ::   alpha = 1./137.036

    real, dimension(12), parameter :: xval0 = (/                       &
    ...
    real*8, save :: ang(7), x0(7), mass(7), intwidth(7), &
         kr(7), kcmr(7), epicmr(7), ppicmr(7), &
         epi2cmr(7), ppi2cmr(7), eetacmr(7), petacmr(7), &
         pgam(7), width(7)
    real*8, save :: br(7,2), pwid(7,2)
    real*8, save :: sigr(5000,7)
    real*8, save :: xvalold(50) = 0.

    real,save :: density, Deltawidth_corr,Deltamass_corr,nucleonmass_corr

    real*8 W !,xb,xth(4)
    real*8 height(7),rescoef(6,4) !,sig_del,sig_21,sig_22,sig_31,sig_32
    real*8 nr_coef(3,4),wdif,wr,sig_nr !,q2temp,sig_4,wdif2
    real*8 k,kcm,ppicm,ppi2cm
    real*8 petacm
    real*8 epicm,epi2cm,eetacm !,spin(7)
    real*8 dip,dip2
    real*8 sig_res,xpr,F1 !,sig_4L
    INTEGER i,j,num,iw !,l,lmax
    real*8 sig_mec !,noverp,fnp_nmc,x,a,b,brp(7,3)
    real*8 w2sv, sigrsv(7) !,q2sv,md,nu,w2p,wp,wdifp,xprp
    real, dimension(2,7) :: F1res
    logical :: first = .true.
  ...
  SUBROUTINE RESMOD507(sf,w2,q2,xval,sigres,sig,sig_res,sig_nr)
    ...
    do i=1,6
       ...
       if (sf.EQ.1) then
          ...
          if(i == 2) height(i) = height(i)*sigmoid(Q2,0.1,0.03)  + (6.335 - 5.03*Q2)*Sigmoid(Q2,0.1,-0.03)
          ! this fixes the unphysical behavior of AT(Q2) for Q2 -> 0
