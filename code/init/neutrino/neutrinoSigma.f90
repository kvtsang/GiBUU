  ...
  subroutine readinput(nuXsectionmode, isExp)
   ...
   !      write(*,*) 'enu=',enu,'elepton=',elepton,'delta_elepton=',delta_elepton
    ...
    case (fixedGamma)
       if (present(isExp)) then
          if (isExp) then
             call traceback("'fixed gamma' and 'exp' does not work.")
          end if
       end if

       ! this routine is called by get_sigma_namelist just to get numax.
       ! we will leave all values at its default.
       ! should be improved in the futeure...

!       call traceback("'fixed gamma' not yet implemented")
  ...
  subroutine Xsec_SigmaMC(eNev, IP, raiseFlag, raiseVal, &
    ...
    ! Monte-Carlo integration over x gives * MC_xmax;
    ! for the integration over y the factor is 1
  ...
  !****************************************************************************
  !****s* neutrinoSigma/Xsec_fixedGamma
  ! NAME
  ! subroutine Xsec_fixedGamma(eNev,IP,raiseFlag,raiseVal,OutPart,sig,flux_enu)
  !
  ! PURPOSE
  ! This subroutine calculates the cross section for EM setup given with
  ! fixed kinematics, read in from namelist 'HiPhoton
  !
  ! INPUTS
  ! * type(electronNucleon_event) :: eNev -- the Lepton-Nucleon Event
  ! * integer             :: IP           -- ID of outgoing hadron/process
  ...
  !
  ! NOTES
  ! raiseFlag, raiseVal, flux_enu make no sense here, so code stops if these
  ! are given.
  !
  ...
  !****************************************************************************
  subroutine Xsec_fixedGamma(eNev, IP, raiseFlag, raiseVal, &
       & OutPart,sig,flux_enu)

    use constants, only: pi
    use eN_eventDefinition
    use eN_event, only: eNev_readFixedKin, eNev_init_Target, eNev_GetKinV
    use neutrinoXsection, only: XsecdCosthetadElepton
    use ParamEP, only: Flux_Bosted
    ...
    type(electronNucleon_event), save :: eNev_init
    logical :: flagOK
    logical, save :: initFlagFunc = .true.
    real :: nu,W,Q2,eps,fT

    raiseVal = 99.9 ! dummy
    if (raiseFlag) &
         call Traceback("raiseflag does not make sense here.")
    if (present(flux_enu)) &
         call Traceback("flux_enu does not make sense here.")

    if (initFlagFunc) then
       if (eNev%idProcess .ne. 1) &
            call Traceback("process must be EM here.")
       if (eNev%idFamily .ne. 1) &
            call Traceback("family must be electron here.")

       call eNev_readFixedKin(eNev_Init, flagOK)
       if (.not.flagOK) call TRACEBACK('kinematics not allowed!')

       call write_electronNucleon_event(eNev_Init, .false., .true.)

       call eNev_GetKinV(eNev_Init, nu,Q2,W,eps=eps,fT=fT)
       fT = fT * eNev_Init%lepton_in%mom(0)*eNev_Init%lepton_out%mom(0)/pi

       write(*,*) 'Flux: fT     ',fT
       write(*,*) 'Flux_Bosted: ',Flux_Bosted(W,Q2,eps)
       write(*,*) 'eps:         ',eps
       write(*,*) 'theta: [°]   ',op_ang(eNev_Init%lepton_in%mom,eNev_Init%lepton_out%mom)

       initFlagFunc = .false.
    end if

    eNev%lepton_in  = eNev_init%lepton_in
    eNev%lepton_out = eNev_init%lepton_out
    eNev%boson      = eNev_init%boson

    call eNev_init_Target(eNev,flagOK=flagOK)
    if (.not.flagOK) call Traceback("init_Target not okay.")

    call XsecdCosthetadElepton(eNev,IP,OutPart, sig)

    call eNev_GetKinV(eNev, nu,Q2,W,eps=eps,fT=fT)
    fT = fT * eNev%lepton_in%mom(0)*eNev%lepton_out%mom(0)/pi
    sig = sig / fT * 1e-3 ! now it is \mu b
    sig = sig / (2*pi) ! ???

  end subroutine Xsec_fixedGamma
