module neutrinoParms
  ...
  !****g* neutrinoParms/T
  ! PURPOSE
  ! target isospin parameter, affects piBG and SIS region
  !
  ! SOURCE
  ...
  !****************************************************************************
  !****g* neutrinoParms/T2p2h
  ! PURPOSE
  ! target isospin parameter, affects only 2p2h
  !
  ! SOURCE
  real, save, public ::  T2p2h = 1
  ! NOTES
  ! * T = 0, 1 , ...
  ! * T = 99 gives T = abs((N-Z)/2)
  !****************************************************************************
...
contains
  ...
  subroutine readInput_neutrino
    ...
    ! * twopiBG_on
    !************************************************************************
    NAMELIST /nl_neutrino2piBack/ NormpiBG,normRES,normBC,Q2cut, &
    ...
    !************************************************************************
    NAMELIST /nl_neweN/ &
         new_eN,new_eNres,ME_ODW,T,T2p2h
    !************************************************************************
  ...
  subroutine readInput_2p2h
    ...
    case (4)
       write(*,'(A)') 'ME4: parametrization of structure functions W1,W2,W3,&
            & W1 from E. Christy'
       !       write(*,*) 'ME_W3   =', ME_W3

    case (5)
       write(*,'(A)') 'ME5: parametrization of structure functions W1,W2,W3,&
            & W1 from Bosted'

    case (6)
       write(*,'(A)') 'ME6: parametrization of structure functions W1,W2,W3,&
  ...
  !****************************************************************************
  !****s* neutrinoParms/VAfact
  ! NAME
  ! subroutine VAfact(ME_ODW,qvecsq,nu,kinfact)
  ! PURPOSE
  ! ???
  ! NOTES
  ! * should be converted into a function (even pure?)
  ! * ME_ODW does not need to be a parameter, since the module variable
  !   could be used
  !****************************************************************************
  subroutine VAfact(ME_ODW,qvecsq,nu,kinfact)
