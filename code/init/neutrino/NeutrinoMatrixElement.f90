...
module NeutrinoMatrixElement
  ...
  ! * 1 = with Mathematica calculated matrix elements (only Delta), no longer maintained
...
contains
  ...
  subroutine readInput_nu_MatrixElement
    ...
    if (which_resonanceModel.eq.0) then
       ...
       write(*,*) 'old matrixelement calculated by Mathematica, no longer usable'
...
! old resonance model, no longer maintained
