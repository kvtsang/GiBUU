...
module ZeroPionAnalysis
  ...
  public :: event_sigma_0pions
  public :: event_dSigma_dE_0pions
...
contains
  ...
  !   if not present then we plot runNumber there
  !
  ...
  subroutine event_sigma_0pions(E,sigmaOut,newInit,runNumber,identifier)
    ...
    if (present(sigmaOut).and.present(runNumber).and.present(newInit)) then
       ...
       if (present(identifier)) then
          sigma(channel,1)=identifier ! column 1 will be running variable
       else
    ...
    do k=0,4
       do i=0,4
          ...
          sigma_name(channel,1)=  &
               intTochar(channel)//': 0pi '//intTochar(k)//'p ' &
               //intTochar(i)//'n'
          ...
          do j=lbound(E,dim=1),ubound(E,dim=1)
             if (sum(E(j)%nParts(1,-1:1)).ne.0) cycle ! no pions
             if (E(j)%nParts(7,1).ne.k) cycle ! "k" protons
             if (E(j)%nParts(7,0).ne.i) cycle ! "i" neutrons
    ...
    do k=0,2
       do i=0,2
          ...
          do j=lbound(E,dim=1),ubound(E,dim=1)
             if (sum(E(j)%nParts(1,-1:1)).ne.2) cycle ! 2 pions
             if (E(j)%nParts(1,-1).ne.k) cycle ! "k" pi-
             if (E(j)%nParts(1,0).ne.i) cycle ! "i" pi0
             if (E(j)%nParts(1,1).ne.l) cycle ! "l" pi+

             perweight=0.
             ! read perweight from the first particle in the list
             if (associated(E(j)%Parts%first))  &
                  perweight=E(j)%Parts%first%V%perweight
    ...
    do k=-1,1
       ...
       do j=lbound(E,dim=1),ubound(E,dim=1)
          if (E(j)%nParts(1,k).ne.1) cycle ! "k" is the charge of the pion
    ...
    do k=0,12
       ...
       channel=channel+1
       sigma_name(channel,1)=intTochar(channel)//': 0pi ' &
            //intTochar(k)//'p X'
       ...
       do j=lbound(E,dim=1),ubound(E,dim=1)
          if (sum(E(j)%nParts(1,-1:1)).eq.0) cycle ! no pions
          if (E(j)%nParts(7,1).eq.k) cycle ! "k" protons
...
!       write(14,formatOut)   '# ',sigma_name(:channel,1),sigma_name(:channel,2)
  ...
  !****************************************************************************
  !****s* ZeroPionAnalysis/event_dSigma_dE_0pions
  ! NAME
  ...
  ! * logical, optional :: sameFileNameIn -- if .true. = always print to same
  !   filenames, else different files for different runs
  ! * type(histogram), dimension(1:numStableParts,-2:2), optional ::
  !   hists,hists1X,hists2X,histsMulti
  !
  ! OUTPUT
  ! * type(histogram), dimension(1:numStableParts,-2:2), optional ::
  !   hists,hists1X,hists2X,histsMulti
    ...
    chargeLoop: do charge=0,1
       if (validCharge(particleIDs(i),charge)) then
          ...
          eventLoop: do j=lbound(E,dim=1),ubound(E,dim=1)
             if (  sum(E(j)%nParts(1,:)).eq.0  ) cycle   ! no pions
                ...
                do particle_number=1,2
                   if ( event_GetParticle(E(j),particleIDs(i),charge,  &
                      ...
                      call addHist(dsigma_dE_2X , ekin  , &
                           & part%perweight*E(j)%nParts(i,charge))
