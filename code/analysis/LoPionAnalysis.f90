...
! double counting.
!******************************************************************************
module LoPionAnalysis

  implicit none
  private

  Public:: DoLoPionAnalysis

  !****************************************************************************
  !****g* LoPionAnalysis/debug
  ! SOURCE
  ...
  !****************************************************************************
  !****g* LoPionAnalysis/dsigma_dOmegadE_switch
  ! SOURCE
...
contains
  ...
  subroutine readinput
    ...
    write(*,*) 'Evaluating dsigma/dOmega and dSigma/dOmega/dE : ',&
         dsigma_dOmegadE_switch
    write(*,*) 'Evaluating 2 Pion output                      : ',&
         twoPi_switch
  ...
  !****************************************************************************
  !****s* LoPionAnalysis/DoLoPionAnalysis
  ! NAME
  ...
  ! --- if .true. then the final output for a series of calls will be done
  ! OUTPUT
  ! * File './pionInduced_xSections.dat' with absorption, reaction and
  !   quasielastic cross sections
  ! * File 'pionInduced_dTheta'//RealToChar(getEkin()*1000.) //'MeV.dat'
  !   with quasielastic cross section : dsigma/dtheta in mB/radian
  !
  ! NOTES
  ...
  subroutine DoLoPionAnalysis(pertParticles,finalFlag)
    ...
    ensembleLoop : do ensemble=1,size(pertParticles,dim=1)
       indexLoop : do index=1,size(pertParticles,dim=2)
          ...
          if (pertParticles(ensemble,index)%ID.eq.pion) then
             ...
             ! If(pertParticles(ensemble,index)%lastCollTime.le.0.0001) then ! particle did not interact
    ...
    ensembleSchleife : do ensemble=1,size(pertParticles,dim=1)
       indexSchleife : do index=1,size(pertParticles,dim=2)
          ...
          !          if((pertParticles(ensemble,index)%ID.eq.pion).and.(pertParticles(ensemble,index)%lastCollTime.ge.0.0001)) then
          ...
          if (pertParticles(ensemble,index)%ID.eq.pion) then
             mom=pertParticles(ensemble,index)%mom
             if (CMFrame) then
    ...
    return
  end subroutine DoLoPionAnalysis


  !****************************************************************************
  !****s* LoPionAnalysis/dSigmadOmegadE
  ! NAME
  ...
  subroutine dSigmadOmegadE(pertParticles,finalFlag)
    ...
    ensembleSchleife : do ensemble=1,size(pertParticles,dim=1)
       indexSchleife : do index=1,size(pertParticles,dim=2)
          ...
          if (pertParticles(ensemble,index)%ID.eq.pion) then
             ! Evaluating the angular distribution
             mom=pertParticles(ensemble,index)%mom(0:3)
             if (CMFrame) then
                !Boost momenta from lab frame to cm frame
                beta=(/0.,0., sqrt((mPi+getekin())**2-mPi**2)/&
                     & (getekin()+mPi+mN)/)
             ...
             if (debug) write(*,*) pertParticles(ensemble,index)%mom(1:3) , 'theta=',theta
...
& getEKin(), '. Number of runs:', numberruns
  ...
  !****************************************************************************
  !****s* LoPionAnalysis/dSigmadOmegadENN
  ! NAME
  ...
  subroutine dSigmadOmegadE_NN(pertParticles,finalFlag)
    ...
    ! Index 1: Ekin of final pion,
    ! Index 2: theta of final pion,
    ! Index 3: charge of final pion
    ...
    ensembleSchleife : do ensemble=1,size(pertParticles,dim=1)
       indexSchleife : do index=1,size(pertParticles,dim=2)
          ...
          if (pertParticles(ensemble,index)%event(1).ne.pert_numbering()) then
             !          if(pertParticles(ensemble,index)%lastCollTime.ge.0.0001) then
             if (pertParticles(ensemble,index)%ID.eq.pion) then
  ...
  !****************************************************************************
  !****s* LoPionAnalysis/dSigmadOmegadE_Resonance
  ! NAME
  ...
  !****************************************************************************
  !****s* LoPionAnalysis/dSigmadOmegadE_pionNuk
  ! NAME
       ...
       indexSchleife : do index=1,size(pertParticles,dim=2)
          ...
          if (pertParticles(ensemble,index)%event(1).ne.pert_numbering()) then
             ...
             if (pertParticles(ensemble,index)%ID.eq.pion) then
                ...
                mom=pertParticles(ensemble,index)%mom(0:3)
                ! Evaluating the angular distribution             mom=pertParticles(ensemble,index)%mom(0:3)
                if (CMFrame) then
                   !Boost momenta from lab frame to cm frame
                   beta=(/0.,0., sqrt((mPi+getekin())**2-mPi**2)/&
                        & (getekin()+mPi+mN)/)
