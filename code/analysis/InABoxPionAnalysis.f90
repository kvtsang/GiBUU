...
module InABoxPionAnalysis
  ...
  public :: InABoxPionAnalysis_count
  public :: InABoxPionAnalysis_eval

contains

  !****************************************************************************
  !****s* InABoxPionAnalysis/InABoxPionAnalysis_count
  ! NAME
  ...
  subroutine InABoxPionAnalysis_count(particleVector,time)
       ...
       do j=lbound(particleVector,dim=2), ubound(particleVector,dim=2)
          if (particleVector(i,j)%ID.eq.pion) then
             ...
             if (particleVector(i,j)%event(1).eq.pert_numbering()) then  ! Initial event in initPion is pert_numbering()
                ...
                velocity=sqrt(Dot_Product(particleVector(i,j)%vel(1:3),particleVector(i,j)%vel(1:3)))
             end if
          end if
       end do
    end do
    ...
    close(321)
  end subroutine InABoxPionAnalysis_count


  !****************************************************************************
  !****s* InABoxPionAnalysis/InABoxPionAnalysis_eval
  ! NAME
  ...
  subroutine InABoxPionAnalysis_eval()
    ...
    if (ios.ne.0) then
       ...
       write(*,*) 'File pion_Numbers.dat is not available!  Critical error in InABoxPionAnalysis_eval. Return!'
       return
    else
       number=0
       gammaMean=0.
  ...
  end subroutine InABoxPionAnalysis_eval



end module InABoxPionAnalysis
